...
static uint8_t eepromWriteData(const PEEPROMStruct pEEPROM) {
  ...
  if (addrOffset) { /* Write address is not page aligned; */
    ...
    i2cAcquireBus(&I2CD1);
    status = i2cMasterTransmitTimeout(&I2CD1, EEPROM_24C02_ADDR, eepromTXBuf, numBytes + 1,
    ...
    if (pEEPROM->dataSize > EEPROM_24C02_PAGE_SIZE) {
      ...
      i2cAcquireBus(&I2CD1);
      status = i2cMasterTransmitTimeout(&I2CD1, EEPROM_24C02_ADDR, eepromTXBuf, EEPROM_24C02_PAGE_SIZE + 1,
        NULL, 0, MS2ST(EEPROM_WRITE_TIMEOUT_MS));
      i2cReleaseBus(&I2CD1);
      if (status != I2C_NO_ERROR) {
        g_i2cErrorInfo.last_i2c_error = i2cGetErrors(&I2CD1);
      ...
      i2cAcquireBus(&I2CD1);
      status = i2cMasterTransmitTimeout(&I2CD1, EEPROM_24C02_ADDR, eepromTXBuf, pEEPROM->dataSize + 1,
        NULL, 0, MS2ST(EEPROM_WRITE_TIMEOUT_MS));
      i2cReleaseBus(&I2CD1);
      if (status != I2C_NO_ERROR) {
        g_i2cErrorInfo.last_i2c_error = i2cGetErrors(&I2CD1);
...
uint8_t eepromLoadSettings(void) {
  ...
  i2cAcquireBus(&I2CD1);
  status = i2cMasterTransmitTimeout(&I2CD1, EEPROM_24C02_ADDR, &addr, 1,
